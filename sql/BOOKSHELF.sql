...
alter table BOOK_CLUB_APPLY add(CLUB_APPLY_DATE DATE DEFAULT LOCALTIMESTAMP); 
alter table BOOK_CLUB_APPLY drop column CLUB_APPLY_DATE;
...
drop table MONTHLY_BOOK;
CREATE TABLE MONTHLY_BOOK (
    MB_CODE VARCHAR2(50) PRIMARY KEY
    , MB_TITLE VARCHAR2(100) NOT NULL
    , MB_WRITER VARCHAR2(100) NOT NULL
    , MB_PUBLISHER VARCHAR2(100) NOT NULL
    , MB_THUMBNAIL VARCHAR2(200) NOT NULL
    , CLUB_CODE VARCHAR2(50) REFERENCES BOOK_CLUB (CLUB_CODE) ON DELETE CASCADE
);
...
alter table BOOK_CLUB drop column CLUB_COMPLIT_CNT;
alter table BOOK_CLUB_APPLY drop column CLUB_APPLY_DATE;
...
alter table BOOK_CLUB_BOARD drop column CB_CMT_COUNT;
--북클럽게시판
